...
def build_rich_popup(row, ip_counts=None):
    ...
    ip = row.get('Ip', 'N/A')
    count = ip_counts.get(ip, 1) if ip_counts else 1
    ...
    pais_display = f"{pais} ({pais_code})" if pais and pais_code else (pais or 'N/A')
    ...
    type_label = f'<span style="color:{infra["circle_color"]};font-weight:bold;">{infra["icon"]} {infra["label"]}</span>'
    ...
    <div style="font-family:'Segoe UI',Arial,sans-serif;font-size:12px;min-width:290px;line-height:1.5;">
        <div style="background:{header_bg};color:white;padding:8px 12px;border-radius:6px 6px 0 0;margin:-1px;">
            <b style="font-size:14px;">{infra['icon']} {ip}</b>
        ...
        <div style="padding:8px 12px;background:#f8fafc;border-radius:0 0 6px 6px;">
            {alert_banner}
            <b>Provedor:</b> {row.get('Ip_Dono', 'N/A')}<br>
            <b>AS:</b> {row.get('Ip_AS', 'N/A')}<br>
            <b>País:</b> {pais_display}<br>
            <b>Região:</b> {row.get('Ip_Regiao', 'N/A')}<br>
            <b>Cidade:</b> {row.get('Ip_Cidade', 'N/A')}<br>
            <b>Data:</b> {row.get('Data', 'N/A')}<br>
            <b>Período:</b> {row.get('Periodo', 'N/A')}<br>
            <b>Classificação:</b> {type_label}<br>
            <div style="margin-top:6px;padding-top:6px;border-top:1px solid #e2e8f0;">
                <a href="https://www.google.com/maps/@{row.get('Ip_Lat','0')},{row.get('Ip_Lon','0')},15z" target="_blank" style="color:#3b82f6;text-decoration:none;font-size:11px;">&#x1F4CD; Google Maps</a>
                &nbsp;|&nbsp;
                <a href="https://www.google.com/maps/@?api=1&map_action=pano&viewpoint={row.get('Ip_Lat','0')},{row.get('Ip_Lon','0')}" target="_blank" style="color:#22c55e;text-decoration:none;font-size:11px;">&#x1F441; Street View</a>
            </div>
        </div>
    </div>"""
...
def build_cluster_popup(rows, ip_counts=None):
    ...
    cidade = rows[0].get('Ip_Cidade', 'N/A')
    regiao = rows[0].get('Ip_Regiao', 'N/A')
    ...
    loc_label = f"{cidade}, {regiao}" + (f" - {pais}" if pais else "")
